...
function Abar_chat(msg)
	...
	elseif msg=="slam" then
		abar.slam = not(abar.slam)
		DEFAULT_CHAT_FRAME:AddMessage('slamtrick is'.. Abar_Boo(abar.slam));
	...
	elseif msg=="info" then
		...
		DEFAULT_CHAT_FRAME:AddMessage('slam- toggle the slamtrick')
...
--					if arg9 then message(arg9) end
  ...
  if (event == "UNIT_INVENTORY_CHANGED" and (not UnitAffectingCombat("player"))) then Abar_Swap() end
...
function abar_spelldir(spellname, prs)
	...
	if abar.swapping and (spellname == "Hamstring") or (spellname == "Sunder Armor") or (spellname == "Execute") or (spellname == "Bloodthirst") or (spellname == "Whirlwind") or (spellname == "Overpower") or (spellname == "Sinister Strike") or (spellname == "Eviscerate") or (spellname == "Hemorrhage") or (spellname == "Backstab") then
		Abar_Swap()
	end
end

function Abar_Slamtrick()
	if(abar.slam)then	
		if(((GetTime() - Abar_Mhr.st) < 0.5) or (left == 0))then
